  ...
  module Strategies
    class Crowd
      class Configuration
        ...
        def token_url(token)
          @token_url.dup << token << '?expand=user'
        end

        def token_verification_url(token)
          @token_url.dup << token
        end
        ...
        def parse_params(options)
          ...
          @token_url              = options[:crowd_token_url] || DEFAULT_TOKEN_URL % options[:crowd_server_url]
          validate_is_url 'token URL', @token_url
